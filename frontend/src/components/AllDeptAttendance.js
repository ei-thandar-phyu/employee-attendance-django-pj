  ...
  const generateAttendanceData = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/all-dept-attendance/?date=${formattedDate}`
      );
    ...
    <div className="dept-attendance">
      ...
      <div className="main-content">
        ...
        {attendanceSummary.map((dept) => (
            <div className="department-section" key={dept.department_name}>
                <h2 className="dept-title">{dept.department_name || "Admins"}</h2>
                <div className="stats-container">
                <div className="stat-card total">
                    <div className="stat-icon"><FaUserClock /></div>
                    <div className="stat-info">
                    <h3>Total Employees</h3>
                    <p>{dept.total_employees}</p>
                    </div>
                </div>
                ...
                <div className="stat-card leave">
                    <div className="stat-icon"><FaUserMinus /></div>
                    <div className="stat-info">
                    <h3>Leave</h3>
