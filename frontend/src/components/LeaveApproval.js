...
const LeaveApproval = () => {
  ...
  const fetchRequestedLeave = async () => {
    try {
        const response = await fetch('http://localhost:8000/api/leave-approval/', {
            method: 'GET',
            credentials: 'include',
        });
                    ...
                    <td>
                      {request.status === 'PENDING' ? (
                        <div className="action-buttons">
                          {showApproveConfirm === request.id ? (
                            <div className="approve-confirm">
                              <button onClick={() => confirmApprove(request.id)} className="confirm">Confirm</button>
                              <button onClick={cancelApprove} className="cancel">Cancel</button>
                            </div>
                          ) : (
                          <button 
                          ...
                          {showRejectConfirm === request.id ? (
                            <div className="reject-confirm">
                              <button onClick={() => confirmReject(request.id)} className="confirm">Confirm</button>
                              <button onClick={cancelReject} className="cancel">Cancel</button>
                            </div>
                          ) : (
                            <button 
                        ...
                        <div className="approval-info">
                          {request.approvedBy && (
                            <div>By: {request.approvedBy}</div>
                          )}
                          {request.approvedAt && (
                            <div>At: {formatDateTime(request.approvedAt)}</div>
                          )}
